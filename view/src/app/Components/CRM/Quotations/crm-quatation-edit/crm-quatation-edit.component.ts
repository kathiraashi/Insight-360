...
@Component({
  selector: 'app-crm-quatation-edit',
  templateUrl: './crm-quatation-edit.component.html',
  styleUrls: ['./crm-quatation-edit.component.css']
})
export class CrmQuatationEditComponent implements OnInit {
      ...
      this.active_route.url.subscribe(u => {
         this.Crm_Service.CrmQuote_View({'Info': Info}).subscribe(response => {
            ...
            if (response['status'] === 200 && ResponseData['Status'] ) {
               ...
               if (this._CompanyName && this._QuoteData && this._CrmConfigList) {
  ...
  // get company contact list
   ...
   simpleEdit() {
      ...
      this.Form.controls['Quote_Ref_Number'].setValue(this._QuoteData['Quote_Ref_Number']);
 ...
 Submit() {
   if (this.Form.valid) {
      ...
      Info = Info.toString();
      this.Crm_Service.CrmQuote_Edit({ 'Info': Info }).subscribe( response => {
         const ResponseData = JSON.parse(response['_body']);
         if (response['status'] === 200 && ResponseData['Status'] ) {
