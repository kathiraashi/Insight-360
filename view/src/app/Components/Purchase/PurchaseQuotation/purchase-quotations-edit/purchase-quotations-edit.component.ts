...
@Component({
  selector: 'app-purchase-quotations-edit',
  templateUrl: './purchase-quotations-edit.component.html',
  styleUrls: ['./purchase-quotations-edit.component.css'],
  providers: [{provide: DateAdapter, useClass: MyDateAdapter}],
})
export class PurchaseQuotationsEditComponent implements OnInit {
   ...
   constructor(
      ...
      this.Purchase_Service.PurchaseQuote_View({'Info': Info}).subscribe(response => {
         ...
         if (response['status'] === 200 && ResponseData['Status'] ) {
            ...
            if (this._VendorList && this.Quote_Details && this.Product_Details) {
               this.simpleEdit();
               this.Loader = false;
            }
         } else if (response['status'] === 400 || response['status'] === 417 && !ResponseData['Status']) {
   ...
   ngOnInit() {
      this.Form = new FormGroup({
         PurchaseQuote_Id: new FormControl(null, Validators.required),
         Vendor_Name: new FormControl(null, Validators.required),
         Contact: new FormControl(null, Validators.required),
         Quote_Ref_Number: new FormControl({value: null, disabled: true}, Validators.required),
   ...
   getPurchaseRequest_ProductDetails(value) {
      ...
      if (value !== '' && value !== null && value !== undefined) {
         ...
         const Data = {'Company_Id': this.Company_Id, 'User_Id': this.User_Id, 'PurchaseRequest_Id': value._id};
         let Info = CryptoJS.AES.encrypt(JSON.stringify(Data), 'SecretKeyIn@123');
         Info = Info.toString();
   ...
   setPurchaseRequestProduct() {
      ...
      this._PurchaseRequestProduct.map(obj => {
         const Group: FormGroup = this.formBuilder.group({
            ...
            Description: new FormControl(obj.Product_Id.Description),
            Price: new FormControl(null, [Validators.required]),
   ...
   simpleEdit() {
      this.Form.controls['PurchaseQuote_Id'].setValue(this.Quote_Details._id);
      this.Form.controls['Vendor_Name'].setValue(this.Quote_Details.Vendor_Name._id);
      this.Form.controls['Contact'].setValue(this.Quote_Details.Contact._id);
      this.Form.controls['Quote_Ref_Number'].setValue(this.Quote_Details.Quote_Ref_Number);
      if (this.Quote_Details.Purchase_Request_Number !== null && this.Quote_Details.Purchase_Request_Number !== '') {
         this.Form.controls['Purchase_Request_Number'].setValue(this.Quote_Details.Purchase_Request_Number._id);
      }
      this.Form.controls['Quote_Date'].setValue(this.toDay);
      this.Form.controls['Subject'].setValue(this.Quote_Details.Subject);
      this.Form.controls['Valid_Date'].setValue(this.Quote_Details.Valid_Date);
      this.Form.controls['Sub_Total'].setValue(this.Quote_Details.Sub_Total);
      this.Form.controls['Tax'].setValue(this.Quote_Details.Tax);
      this.Form.controls['Total'].setValue(this.Quote_Details.Total);
      if (this.Quote_Details.Global_Product_Tax !== null) {
         this.Form.controls['Global_Product_Tax'].setValue(this.Quote_Details.Global_Product_Tax.map(x => x._id));
         this.Form.controls['Overall_Global_Tax'].setValue(parseFloat(this.Quote_Details.Overall_Global_Tax));
      }
       // item set value
       this.items = this.Form.get('items') as FormArray;
 ...
 // ************************************************** Tax *************************************************************
   ...
   Submit() {
      if (this.Form.valid) {
         ...
         Info = Info.toString();
         this.Purchase_Service.PurchaseQuote_Edit({ 'Info': Info }).subscribe( response => {
            const ResponseData = JSON.parse(response['_body']);
            if (response['status'] === 200 && ResponseData['Status'] ) {
