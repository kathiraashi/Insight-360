...
export class PurchaseOrdersCreateComponent implements OnInit {
  Active_Tab = 'Product_Details';
   ...
   global_tax_amount = 0;
   _ConfigList: any;
   Quote_Ref_Number: any;
   PurchaseQuote_Ref_Number: any;
   _PurchaseQuoteProduct: any;
   _PurchaseQuote: any;
   constructor(
      ...
      this.Purchase_Service.PurchaseQuote_List({'Info': Info}).subscribe( response => {
         ...
         if (response['status'] === 200 && ResponseData['Status'] ) {
            const CryptoBytes  = CryptoJS.AES.decrypt(ResponseData['Response'], 'SecretKeyOut@123');
            const DecryptedData = JSON.parse(CryptoBytes.toString(CryptoJS.enc.Utf8));
            this.PurchaseQuote_Ref_Number = DecryptedData;
         } else if (response['status'] === 400 || response['status'] === 417 && !ResponseData['Status']) {
            this.Toastr.NewToastrMessage({Type: 'Error', Message: response['Message']});
         } else if (response['status'] === 401 && !ResponseData['Status']) {
            this.Toastr.NewToastrMessage({ Type: 'Error',  Message: ResponseData['Message'] });
   ...
   ngOnInit() {
      this.Form = new FormGroup({
         ...
         Purchase_Quote_Ref_Number: new FormControl(null),
   ...
   getPurchaseQuote_ProductDetails(value) {
      this.SubLoader = true;
      if (value !== '' && value !== null) {
         ...
         const Data = {'Company_Id': this.Company_Id, 'User_Id': this.User_Id, 'PurchaseQuote_Id': value};
         let Info = CryptoJS.AES.encrypt(JSON.stringify(Data), 'SecretKeyIn@123');
         Info = Info.toString();
         ...
         if (response['status'] === 200 && ResponseData['Status'] ) {
            const CryptoBytes  = CryptoJS.AES.decrypt(ResponseData['Response'], 'SecretKeyOut@123');
            const DecryptedData = JSON.parse(CryptoBytes.toString(CryptoJS.enc.Utf8));
            this._PurchaseQuote = DecryptedData.Quote_Details;
            this._PurchaseQuoteProduct = DecryptedData.Product_Details;
            // console.log(this._PurchaseQuoteProduct);
            if (this._PurchaseQuoteProduct) {
            //    this.Form.controls['Quote_Ref_Number'].setValue(this._PurchaseQuoteProduct.Quote_Ref_Number);
               this.setPurchaseQuoteProduct();
            }
   ...
   setPurchaseQuoteProduct() {
      ...
      this.Form.controls['Total'].setValue(null);
      this.Form.controls['Global_Product_Tax'].setValue(null);
      this._PurchaseQuoteProduct.map(obj => {
         const inlineProductTax = obj.Product_Tax.map(x => x._id);
         const Group: FormGroup = this.formBuilder.group({
            ...
            Price: new FormControl(parseFloat(obj.Price)),
            Quantity: new FormControl(parseFloat(obj.Quantity), [Validators.required]),
            Taxable_Amount: new FormControl(obj.Tax_Amount),
            Product_Tax: new FormControl(inlineProductTax),
            Overall_Inline_Tax: new FormControl(obj.Overall_Inline_Tax),
            Tax_Amount: new FormControl(obj.Tax_Amount),
            Product_Total: new FormControl(obj.Product_Total),
         });
         this.items.push(Group);
         this.FilterProduct();
      });
      const globalProductTax = this._PurchaseQuote.Global_Product_Tax.map(y => y._id);
      // this.Form.controls['Sub_Total'].setValue(parseFloat(this._PurchaseQuote.Sub_Total));
      this.Form.controls['Global_Product_Tax'].setValue(globalProductTax);
      // this.Form.controls['Tax'].setValue(parseFloat(this._PurchaseQuote.Tax));
      // this.Form.controls['Total'].setValue(parseFloat(this._PurchaseQuote.Total));
