...
export class PurchaseRequestApproveComponent implements OnInit {
   ...
   createItems(): FormGroup {
      return this.formBuilder.group({
         ...
         Approved_Quantity: new FormControl(null, Validators.required)
      });
   }
   // add items to bill
   addItem(): void {
   ...
   simpleApprove() {
      ...
      this.Product_Details.map(obj => {
         const Group: FormGroup = this.formBuilder.group({
            ...
            Approved_Quantity: new FormControl(null),
         });
         this.items.push(Group);
         this.FilterProduct();
      });
   }
   copyApproveValue(value) {
      const length = this.Form.controls['items']['controls'].length;
      if (value === true) {
         for (let i = 0; i < length; i++) {
            this.Form.controls['items']['controls'][i]['controls'].Approved_Quantity.setValue(this.Form.controls['items']['controls'][i]['controls'].Required_Quantity.value);
            this.Form.controls['items']['controls'][i]['controls'].Approved_Quantity.disable();
         }
      } else {
         for (let j = 0; j < length; j++) {
            this.Form.controls['items']['controls'][j]['controls'].Approved_Quantity.setValue(null);
            this.Form.controls['items']['controls'][j]['controls'].Approved_Quantity.enable();
         }
      }
   }
   Submit() {
      ...
      if (this.Form.valid) {
         ...
         Info = Info.toString();
         this.Purchase_Service.PurchaseRequest_Approve({ 'Info': Info }).subscribe( response => {
            const ResponseData = JSON.parse(response['_body']);
            if (response['status'] === 200 && ResponseData['Status'] ) {
