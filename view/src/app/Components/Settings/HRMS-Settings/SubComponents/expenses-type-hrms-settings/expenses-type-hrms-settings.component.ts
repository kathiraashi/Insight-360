...
export class ExpensesTypeHrmsSettingsComponent implements OnInit {
   ...
   // Create New Expenses Type
      CreateExpensesType() {
         ...
         this.bsModalRef = this.modalService.show(ModelExpensestypeHrmssettingsComponent, Object.assign({initialState}, { ignoreBackdropClick: true, class: '' }));
   ...
   // Delete Expenses Type
      DeleteExpensesType(_index) {
         ...
         this.bsModalRef.content.onClose.subscribe(response => {
            if (response.Status) {
               const Data = { 'Expenses_Type_Id' :  this._List[_index]._id, 'Modified_By' : this.User_Id };
               let Info = CryptoJS.AES.encrypt(JSON.stringify(Data), 'SecretKeyIn@123');
               Info = Info.toString();
