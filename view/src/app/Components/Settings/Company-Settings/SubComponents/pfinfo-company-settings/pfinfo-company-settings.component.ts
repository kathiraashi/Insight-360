...
export class PFInfoCompanySettingsComponent implements OnInit {
  ...
  constructor(private modalService: BsModalService,
    ...
    public PermissionCheck: PermissionsCheckService) {
      ...
      Info = Info.toString();
      this.Service.PfInfo_List({'Info': Info}).subscribe( response => {
...
CreatePfInfo() {
 ...
 ViewPfInfo(_index) {
  ...
  DeletePfInfo(_index) {
    ...
    this.bsModalRef.content.onClose.subscribe(response => {
      if (response.Status) {
         const Data = { 'Pf_Info_Id' :  this._List[_index]._id, 'Modified_By' : this.User_Id };
         let Info = CryptoJS.AES.encrypt(JSON.stringify(Data), 'SecretKeyIn@123');
         Info = Info.toString();
         this.Service.PfInfo_Delete({'Info': Info}).subscribe( returnResponse => {
            const ResponseData = JSON.parse(returnResponse['_body']);
            if (returnResponse['status'] === 200 && ResponseData['Status'] ) {
               this._List.splice(_index, 1);
               this.Toastr.NewToastrMessage( {  Type: 'Warning', Message: 'PfInfo Successfully Deleted'  } );
