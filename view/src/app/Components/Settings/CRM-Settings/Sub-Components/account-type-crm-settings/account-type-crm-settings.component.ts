...
export class AccountTypeCrmSettingsComponent implements OnInit {
   ...
   constructor(
      ...
      public PermissionCheck: PermissionsCheckService
         ...
         Info = Info.toString();
         this.Service.Account_Type_List({'Info': Info}).subscribe( response => {
   ...
   // Create New Account Type
      CreateAccountType() {
         ...
         this.bsModalRef = this.modalService.show(ModelAccouttypeCrmSettingsComponent, Object.assign({initialState}, {ignoreBackdropClick: true, class: '' }));
  ...
  // View Account Type
      ViewAccountType(_index) {
         const initialState = {
            Type: 'View',
            Data: this._List[_index]
         };
         this.bsModalRef = this.modalService.show(ModelAccouttypeCrmSettingsComponent, Object.assign({initialState}, { class: '' }));
      }
   // Delete Account Type
      DeleteAccountType(_index) {
         const initialState = {
            Text: 'Account Type'
         };
         ...
         this.bsModalRef.content.onClose.subscribe(response => {
            if (response.Status) {
               const Data = { 'Account_Type_Id' :  this._List[_index]._id, 'Modified_By' : this.User_Id};
               let Info = CryptoJS.AES.encrypt(JSON.stringify(Data), 'SecretKeyIn@123');
               Info = Info.toString();
               this.Service.Account_Type_Delete({'Info': Info}).subscribe( returnResponse => {
                  const ResponseData = JSON.parse(returnResponse['_body']);
                  if (returnResponse['status'] === 200 && ResponseData['Status'] ) {
                     this._List.splice(_index, 1);
                     this.Toastr.NewToastrMessage( {  Type: 'Warning',  Message: 'Account Type Successfully Deleted' } );
