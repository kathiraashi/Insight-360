...
export class ModelLeavetypeHrmssettingsComponent implements OnInit {
   ...
   ngOnInit() {
      ...
      // If Edit New Leave type
         if (this.Type === 'Edit') {
            this.Form = new FormGroup({
               Leave_Type: new FormControl(this.Data.Leave_Type, {validators: Validators.required,
                                                                  asyncValidators: [ this.LeaveType_AsyncValidate.bind(this) ],
                                                                  updateOn: 'blur' } ),
   ...
   // onSubmit Function
      ...
      LeaveType_AsyncValidate( control: AbstractControl ) {
         const Data = { Leave_Type: control.value, Company_Id: this.Company_Id, User_Id: this.User_Id  };
